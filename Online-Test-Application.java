...
class OnlineTestApplication extends JFrame implements ActionListener {
	...
	OnlineTestApplication(String s) {
		super(s);
		label = new JLabel();
		add(label);
		bg = new ButtonGroup();
	...
	void set() {
		...
		if (current == 2) {
			label.setText("Que3: Which of the following is a marker interface?");
		...
		if (current == 6) {
			label.setText("Que7: Which of the following is used to find and fix bugs in the Java programs?");
		...
		if (current == 8) {
			label.setText("Que9: Which keyword is used for accessing the features of a package?");
	...
	public static void main(String s[]) {
		new OnlineTestApplication("Online Test Application");
	}
